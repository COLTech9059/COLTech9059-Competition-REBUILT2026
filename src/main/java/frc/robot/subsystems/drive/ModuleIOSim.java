...
public class ModuleIOSim implements ModuleIO {
  ...
  private static final double DRIVE_KI = 0.00;
  private static final double DRIVE_KD = 0.00;
  private static final double DRIVE_KS = 0.0;
  private static final double DRIVE_KV_ROT =
      0.91035; // Same units as TunerConstants: (volt * secs) / rotation
  private static final double DRIVE_KV = 1.0 / Units.rotationsToRadians(1.0 / DRIVE_KV_ROT);
  private static final double TURN_KP = 8; // .1
  private static final double TURN_KI = 0;
  private static final double TURN_KD = 0.0; // .01
  ...
  private PIDController driveController = new PIDController(DRIVE_KP, DRIVE_KI, DRIVE_KD);
  private PIDController turnController = new PIDController(TURN_KP, TURN_KI, TURN_KD);
  private double driveFFVolts = 0.0;
  private double driveAppliedVolts = 0.0;
  private double previousAppliedVolts = 0.0;
  ...
  public void simulationPeriodic() {
    ...
    double fixedAppliedVolts = driveAppliedVolts / 100 * 12;
    Logger.recordOutput("ModuleIOSim/DriveAppliedVolts", driveAppliedVolts);
    Logger.recordOutput("ModuleIOSim/FixedAppliedVolts", fixedAppliedVolts);

    // Apply voltages
    // if (fixedAppliedVolts > -0.6 && fixedAppliedVolts < 0.6) {
    //   driveSim.setInputVoltage(-2 * fixedAppliedVolts);
    // }
    /*else*/ driveSim.setInputVoltage(MathUtil.clamp(fixedAppliedVolts, -12.0, 12.0));
    ...
    previousAppliedVolts = driveAppliedVolts;
  }
